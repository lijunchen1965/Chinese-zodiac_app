...
def get_zodiac(birth_date):
    ...
    # Check if year is in lunar_new_year_dates
    if birth_year not in lunar_new_year_dates:
        return "Unknown"

    lunar_new_year_date = datetime.datetime.strptime(f"{birth_year} {lunar_new_year_dates[birth_year]}", "%Y %B %d")
...
def main():
    ...
    try:
        ...
        if zodiac == "Unknown":
            ...
            print(f"Your Chinese Zodiac Animal is: {zodiac}")
